...
const NetworkLogs: React.FC = () => {
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await api.post('/scan/network', formData, {
  ...
  return (
    <AdminLayout>
      <Box sx={{ mt: 4, px: 3 }}>
        <Typography variant="h4" fontWeight="bold" gutterBottom>🌐 Network Logs Analyzer</Typography>
        ...
        >
          {loading ? <CircularProgress size={20} /> : 'Scan Logs'}
        </Button>
